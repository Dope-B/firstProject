...
#### 피드백
 - 처음 만든 프로젝트다 보니 객체의 함수 구성이 적절하지 않다.
 - 게임을 관리하는 객체가 없어 메인 함수가 너무 지저분하고 확장성이 전혀 고려되지 않았다.
 - 구조물의 위치에 대해 하드코딩 되어 수정 하기 어렵다.
 - 몬스터와의 충돌처리나 휴식 상태에서의 구현이 불완전하다.
 - 리소스 파일 경로가 절대경로로 되어있어 타 PC에서 실행이 불가능함
